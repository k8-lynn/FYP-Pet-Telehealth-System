...
export const NotificationProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (!usr_id) {
      console.log(
        "⚠️ No userid found in sessionStorage, skipping socket connection"
      );
      return;
    }

    console.log(
      "🔌 Connecting to Socket.IO for userid:",
      usr_id,
      "userType:",
      userType
    );
    ...
    newSocket.on("callUser", ({ from, signal, name, chatId, petInfo }) => {
      // ✅ ADD petInfo
    ...
    newSocket.on("newNotification", (notification) => {
      ...
      const isOnRemindersPage =
        window.location.pathname === "/petowner-reminders";
  ...
  // ✅ ADD THIS - Ensure joinUser is called when userid becomes available
  React.useEffect(() => {
    const usr_id = sessionStorage.getItem("userid");

    if (socket && usr_id) {
      console.log("🔌 NotificationProvider: Joining user room for:", usr_id);
      socket.emit("joinUser", usr_id);
    }
