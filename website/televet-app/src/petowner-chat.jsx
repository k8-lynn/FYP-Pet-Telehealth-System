...
const PetOwnerChat = () => {
  ...
  const [searchResults, setSearchResults] = useState({ vets: [], messages: [] });
...
React.useEffect(() => {
  ...
  const handleSearch = async (query) => {
    ...
    const vetResults = chats.filter(chat => 
    ...
    try {
      const response = await fetch(
        `http://localhost:5000/api/chat/search-messages?query=${encodeURIComponent(query)}&usr_id=${userid}&role=pp`
      );
                ...
                <div className="search-results">
                  ...
                  <div className="search-section">
                    ...
                    {searchResults.vets.length > 0 ? (
                      ...
                      <p className="search-no-results">No veterinarians found</p>
                    )}
                  </div>
